...
# Эта штука отдельно и капслоком: ключ ноды по умолчанию протухает через
# 180 дней, после чего машина молча выпадает из тайлнета. Для сервера, к
# которому нельзя подъехать, это самый обидный способ потерять доступ.
echo
warn "ОБЯЗАТЕЛЬНО, ОДИН РАЗ: в https://login.tailscale.com/admin/machines"
warn "для ноды '${HOSTNAME_TS}' -> ... -> Disable key expiry."
warn "Иначе через 180 дней доступ через Tailscale пропадёт сам по себе."
